"""v2m1_maxwell_loop -- M1' 主跑(轮 2,交付物 3):冻结自旋 1 候选,同一 rule、
每 L 一次运行喂全部门,逐 L in {16, 24, 32, 48},全部经 M0' 统一评估器出数。
...
一、"同一次运行"操作定义(写死):每 L 一条连续运行记录,相位表:
  P0 无源波段    T_WAVE=1024        通用随机初条(SEED_WAVE=20260726+L),8 trials,
  ...
  P5 动源段      T_MOVE=8*HOPS(L)   +q 以 v=0.25c(8 步 1 胞跳,守恒沉积)沿 +y;
                                    锥判读(G4b)+ 电荷守恒(G5)
  P6 波包段      T_PK(L)            6 非 Nyquist 判据 k 波包注入 trial 0..5
...
三、仪器常数(预飞/试飞标定后冻结;非门阈值):
  W_SPONGE={16:4,24:5,32:6,48:8}; SIGK={16:0.45,24:0.40,32:0.35,48:0.30};
  T_PK={16:6000,24:7500,32:9500,48:12000}(试飞实测迟时 e-fold 200/280/380/510 步
  + 余量);HOPS={16:2,24:3,32:4,48:4};PK_STRIDE=8;A_PK=1.0;Q_SRC=1.0。
  L=64:按实测每步成本推算 >15 分钟单独占用,超单脚本 30 分钟预算,不跑,如实注明。
...
措辞红线(§8 红线 10):本件即使全门 PASS 也不宣告 M1' PASS——判定须
全门 + 全炮(轮 3)+ epsilon=1 实测 + 双环境 四者同时,收口在车道A 复核 + PI。
任一门 FAIL 即停,写追因(禁调参),报车道A。fp64;增量写盘;沙盒位 PENDING。

Run:  RULESPACE_BACKEND=numpy .venv/bin/python experiments/v2m1_maxwell_loop.py
      (writes data/results/v2m1_maxwell_loop.json)
"""
...
OUT = os.path.join(ROOT, "data", "results", "v2m1_maxwell_loop.json")
...
TRIALS = 8
T_WAVE = 1024
SEED_WAVE_BASE = 20260726          # seed(L) = SEED_WAVE_BASE + L
...
T_PK = {16: 6000, 24: 7500, 32: 9500, 48: 12000}
...
def run_one_L(L, dict_step, log):
    ...
    out = {"L": L, "seed_wave": seed, "trials": TRIALS,
           "phases": [], "instrument": {
               "W_sponge": w, "sigk": SIGK[L], "T_pk": T_PK[L],
    ...
    log("  P6 pk %ds  tau=%s resid=%s floor=%.1e"
        % (time.time() - t0, taus,
...
def main():
    ...
    payload = {
        "register": "v2m1-maxwell-loop (M1' 主跑轮 2;任务书 §3 交付物 3)",
        "status": "RUNNING", "backend": "numpy (fp64)",
        "authority": ["docsv2/v2-任务书-M1-Maxwell闭环.md §3/§6/§8",
    ...
    log("v2m1 maxwell loop: M1' 主跑(轮 2)")
    log("=" * 74)
    ...
    write_json(payload)
    log("[cert] pinned hash(%d): %s; M0'注册表: %s; construction: %s; 前提: %s"
        % (len(checked), hok, hz0["pass"], cons_ok, pre_ok))
    ...
    gc.set_gate("G7_stability",
                ...
                diagnostics={"gate": G7_RADIUS_GATE,
                             "endurance": "PENDING(轮 3 负控列,预算内预注册)",
    ...
    payload["L64_note"] = ("64^3 未跑:按实测每步成本(48^3 全相位 ~7 分钟)"
                           "推算 64^3 全相位 >15 分钟,叠加本脚本已用时超 30 "
                           "分钟单脚本预算——如实注明(任务书 §8 红线 8)")
    payload["sandbox"] = {"status": "PENDING",
                          "note": "双环境判据字段比对归车道A 复核(判据字段 "
                                  "1e-12;谱小分量/噪声底派生量报告级)"}
    payload["endurance"] = {"status": "PENDING(轮 3 负控列)"}
    payload["verdict"] = (
        ("主跑逐 L 全门落位(宿主):G1-G8 本轮全部 PASS。按红线 10 不宣告 "
         "M1' PASS(尚缺:轮 3 全炮组 + 双环境 + 车道A 复核 + PI 收口);"
         "epsilon_DOF 实测 = %s(hand_built=None,与 D1 锚一致,实测非声明)。"
         % ([eps_min, eps_max])) if all_pass else
        ("存在 FAIL 门:%s——按任务书 §6 停手,本件即为追因材料的原始读数层,"
         "报车道A;禁止调参重跑。"
         % [k for k, v in verdicts.items() if v != "PASS"]))
    payload["status"] = "DONE" if all_pass else "FAIL-STOP"
