"""cp1_v4_L3v3 -- CP1-v4 assembly ladder, LEVEL 3 THIRD ATTACK (lane B):
the walk-trace symplectic kernel, everything else frozen.

SINGLE CHANGE FROM L3-v2: replace the ideal half-angle stiffness by the local
walk-trace/Wilson blend

    a_v3 = (1-q^3) a_tr + q^3 a_kappa,
    q = (1/3) sum_i sin^2(k_i/2).

The flat low-pass keeps the full walk dispersion to better than the J5 gate
on the registered low-k set, while the Wilson return to a_kappa removes the
walk trace's UV near-zero foldbacks.  The pair keeps its static double pole.

Inherited L3-v2 record follows:
...
# ===========================================================================
def walk_trace_action(h):
    """Apply tr(U_walk) to scalar fields with the walk's local stencil."""
    q0 = np.zeros(h.shape + (2,), complex)
    q1 = np.zeros_like(q0)
    q0[..., 0] = h
    q1[..., 1] = h
...
def lap_ideal(h):
    """Original ideal half-angle Laplacian, symbol -a_kappa."""
    out = -6.0 * h
    for ax in (-3, -2, -1):
...
def q_apply(h):
    """Local positive operator q=(1/3) sum sin^2(k_i/2)."""
    return -lap_ideal(h) / (12.0 * CT2)


def lap_kappa(h):
    """Finite radius-4 local v3 Laplacian with symbol -a_v3."""
    lap_tr = walk_trace_action(h) - 2.0 * h
    correction = lap_ideal(h) - lap_tr
    for _ in range(3):
        correction = q_apply(correction)
    return lap_tr + correction
...
def pair_omega(kl):
    """Pair frequency: exactly the full matter-walk shell at every k."""
    a = a_of(kl)
    return 2.0 * math.asin(min(1.0, math.sqrt(max(0.0, a)) / 2.0))


def a_of(kl):
    """Walk-trace stiffness with a cubic-flat Wilson UV return."""
    kl = np.asarray(kl)
    atr = float(2.0 - np.real(np.trace(L2.walk_symbol(kl))))
    sk = sum(math.sin(float(k) / 2.0) ** 2 for k in kl)
    ak = 4.0 * CT2 * sk
    q3 = (sk / 3.0) ** 3
    return (1.0 - q3) * atr + q3 * ak
...
def v2_calibrate(N, full_bz_stability=True):
    """Spectrally calibrate the walk-trace pair.

    The v2 closed-form point assumes the ideal pair shell and is not stable
    after the walk-trace substitution.  Recalibration is permitted by the
    assembly protocol, so scan a declared kappa1 grid [0.90,1.00] and the
    same dyadic mu ladder, then choose the stable point with the fastest true
    augmented-spectrum contraction.  N=16 candidates are finally certified
    on the complete BZ; larger boxes use the declared sample.
    """
    s2_all = []
    for aa in range(N):
        for bb in range(N):
    ...
    mu_cf = cal["mu_z"]
    screen_ks = sample_ks(N, n_rand=700, seed=7)
    candidates = []
    ladder = []
    k1_grid = sorted(set([float(cal["kappa1"])]
                         + [0.70 + 0.05 * q for q in range(7)]
                         + [0.90 + 0.01 * q for q in range(11)]))
    for k1_try in k1_grid:
        for j in range(0, 12):
            mu = mu_cf * 2.0 ** (-j)
            worst, rate = bz_spectrum(N, mu, k1_try, ks=screen_ks)
            row = {"j": j, "mu": mu, "kappa1": k1_try,
                   "max_abs_lambda_screen": worst,
                   "rate_slowest_screen": rate}
            ladder.append(row)
            if worst <= 1.0 + 1e-12:
                candidates.append((rate, k1_try, j, mu))
    assert candidates, "v3_calibrate: no stable point on declared grid"
    chosen = None
    for _, k1_try, j, mu in sorted(candidates):
        ks_final = None if full_bz_stability else screen_ks
        worst, rate = bz_spectrum(N, mu, k1_try, ks=ks_final)
        if worst <= 1.0 + 1e-12:
            chosen = (j, mu, k1_try, worst, rate)
            break
    assert chosen is not None, "v3_calibrate: screened points fail final BZ"
    j, mu, k1, worst, rate = chosen
    out = dict(cal)
    out.update({"mu_closed_form": mu_cf, "mu_closed_form_unstable":
                True, "kappa1_closed_form": cal["kappa1"],
                "mu_z": float(mu), "kappa1": float(k1),
                "dyadic_j": j, "ladder": ladder,
                "max_abs_lambda": worst, "rate_pred_true_spectrum": rate,
                "stability_scan": "full BZ" if full_bz_stability
                else f"declared sample ({len(screen_ks)} k)",
                "selection": "min rate over stable declared kappa1 grid "
                "(closed-form union coarse/fine grid) x "
                "mu_closed_form*2^-j, j=0..11"})
    return out
...
# ===========================================================================
def cert_T10(N=16):
    """Band census and exact walk-shell identity, including oblique k."""
    ...
    for k in ks:
        ...
        ew = np.linalg.eigvals(L2.walk_symbol(k))
        w_sh = float(np.sort(np.abs(np.angle(ew)))[0])
    ...
    j5_gate_dev = max(v["j5_dev_pred"] for v in j5_tab.values())
    ok = (nbands == {2} and worst_mod < 1e-9 and j5_gate_dev < 1e-2)
    return {"bands": sorted(nbands), "eig_mod_dev": worst_mod,
            "shell_dev": worst_shell,
            "registered_j5_dev_max": float(j5_gate_dev),
            "min_dist_eig_to_1_smallest_k": float(min_gap),
            "j5_oracle_pair_vs_walk": j5_tab, "pass": bool(ok)}


def cert_T11():
    """Static 1/a_tr double pole by actual damped-probe iteration."""
    rng = np.random.default_rng(0)
    ks = [np.array([0.5, 0, 0]), np.array([0.35, 0.35, 0.35]),
          np.array([0.7, 0.2, -0.4])] + [rng.uniform(-1.1, 1.1, 3)
                                         for _ in range(40)]
...
def cert_T13(N=24):
    ...
    denom = np.empty((N, N, N))
    for ia, ka in enumerate(kk):
        for ib, kb in enumerate(kk):
            for ic, kc in enumerate(kk):
                denom[ia, ib, ic] = a_of((ka, kb, kc))
...
if __name__ == "__main__":
    ...
    print("CP1-v4 L3-v3 -- walk-trace (h,pi) kernel, everything else "
          "frozen (lane B)")
    print("=" * 74)
    out = {"backend": B.NAME, "th0": TH, "c_cone": C, "G_frozen": G_FROZEN,
           ...
           "declared": [
               "single change from v2: time-staggered (h,pi) pair uses the "
               "local Wilson blend a_v3=(1-q^3)a_tr+q^3 a_kappa, q="
               "sum sin^2(k_i/2)/3; low-k follows the walk inside J5 while "
               "the UV returns to the certified kappa shell; "
    ...
    print(f"\nL3-v3 VERDICT: {'ALL GATES PASS' if all_pass else 'NOT ALL PASS'}")
    ...
    with open(os.path.join(DIR, "cp1_v4_L3v3_results.json"), "w") as fh:
        json.dump(_san(out), fh, indent=1)
    print(f"\nscript sha256 = {out['script_sha256']}")
    print(f"total {out['total_seconds']:.0f}s   wrote cp1_v4_L3v3_results.json")
