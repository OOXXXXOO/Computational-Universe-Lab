"""Task-8 contract tests for the isolated A_FLAT route."""
...
ROUTE_PATH = REPOSITORY_ROOT / "experiments/v3m0_b7_schema_lab/a_flat.py"
ROUTE_ID = "A_FLAT"
...
def _route_module():
    return importlib.import_module("experiments.v3m0_b7_schema_lab.a_flat")
...
def _resign_transcript(transcript: dict[str, object]) -> bytes:
...
def test_a_flat_exports_exact_bytes_only_api_and_passes_frozen_static_scan() -> None:
    route = _route_module()
...
def test_a_flat_roundtrips_each_exact_terminal_presence_case(case_id: str) -> None:
    route = _route_module()
    ...
    assert wire == _canonical(json.loads(wire))
    assert route.verify_and_decode_route_wire(wire) == source
    assert route.encode_normalized_transcript(source) == wire
    assert route.verify_and_decode_route_wire(wire) == source
...
def test_a_flat_rejects_every_non_bytes_input(api_name: str, payload: object) -> None:
    route = _route_module()
...
def test_a_flat_rejects_noncanonical_duplicate_concat_and_invalid_json(mutate) -> None:
    route = _route_module()
...
def test_a_flat_rejects_unknown_or_missing_transcript_and_wire_fields(
    operation: str,
) -> None:
    route = _route_module()
...
@pytest.mark.parametrize(
    ("field", "value"),
    (
        ("transcript_schema_version", "wrong"),
        ("case_id", "eighth_case"),
        ("terminal_tag", "eighth_case"),
    ),
)
def test_a_flat_rejects_schema_and_case_domain_drift(field: str, value: object) -> None:
    route = _route_module()
    transcript = _transcript("success")
    ...
    reason = (
        "SCHEMA_MISMATCH"
        if field == "transcript_schema_version"
        else "PRESENCE_MISMATCH"
    )
...
def test_a_flat_rejects_every_self_hash_layer_drift(attack: str) -> None:
    route = _route_module()
    transcript = _transcript("success")
...
def test_a_flat_distinguishes_lineage_shas_from_declared_self_hashes() -> None:
    route = _route_module()
    transcript = _transcript("success")
    ...
    _assert_rejected(
        route.encode_normalized_transcript,
        _resign_transcript(transcript),
        "HASH_MISMATCH",
    )


@pytest.mark.parametrize("case_id", [row[0] for row in CASE_ROWS])
def test_a_flat_rejects_presence_terminal_failure_and_trace_mutations(
    case_id: str,
) -> None:
    route = _route_module()
    ...
    trace = copy.deepcopy(transcript)
    trace["callback_trace"] = list(reversed(trace["callback_trace"]))
    _seal(trace, "experimental_sha")
    if len(trace["callback_trace"]) > 1:
        _assert_rejected(
            route.encode_normalized_transcript,
            _canonical(trace),
    ...
    if transcript["actual_branch_attempt"] is not None:
        ...
        if failure["actual_branch_attempt"]["failure"] != next(
            row[2] for row in CASE_ROWS if row[0] == case_id
        ):
            _assert_rejected(
                route.encode_normalized_transcript,
                _canonical(failure),
...
def test_a_flat_rejects_nested_body_and_evidence_splices(attack: str) -> None:
    route = _route_module()
    transcript = _transcript("success")
    if attack == "shell_reference_splice":
        transcript["shell_outcome"]["reference_outcome"]["outcome_sha"] = "0" * 64
        _seal(transcript["shell_outcome"], "outcome_sha")
    elif attack == "completed_values_splice":
        transcript["actual_completed_response"]["values"] = copy.deepcopy(
            transcript["matched_ablated_branch_attempt"]["response_values"]
        )
        transcript["actual_completed_response"]["values"]["values_wire"] = [[1.0, 0.0]]
        _seal(transcript["actual_completed_response"]["values"], "tensor_sha")
        _seal(transcript["actual_completed_response"], "response_sha")
    elif attack == "completed_audit_splice":
        transcript["actual_completed_response"]["bridge_audit"] = copy.deepcopy(
            transcript["matched_ablated_branch_attempt"]["bridge_audit"]
        )
        _seal(transcript["actual_completed_response"], "response_sha")
    elif attack == "completed_shell_splice":
        transcript["actual_completed_response"]["shell_manifest_sha"] = "0" * 64
        _seal(transcript["actual_completed_response"], "response_sha")
    else:
        transcript["ordered_leaf_digests"][0]["call_ordinal"] = 1
    _seal(transcript, "experimental_sha")
    _assert_rejected(
        route.encode_normalized_transcript,
        _canonical(transcript),
...
def test_a_flat_wire_binds_flat_slots_and_detects_decoded_evidence_drift() -> None:
    route = _route_module()
    source = _canonical(_transcript("success"))
    wire = json.loads(route.encode_normalized_transcript(source))
    assert wire["wire_schema_version"] == WIRE_SCHEMA_ID
    assert wire["route_id"] == ROUTE_ID
    assert wire["terminal_tag"] == "success"
    assert wire["presence_bits"] == "111111111"
    assert wire["transcript"] == json.loads(source)

    wire["transcript"]["reference_outcome"]["status"]["defined"] = False
    _seal(wire["transcript"]["reference_outcome"], "outcome_sha")
    _seal(wire["transcript"], "experimental_sha")
    wire["transcript_sha"] = wire["transcript"]["experimental_sha"]
    _seal(wire, "wire_sha")
    _assert_rejected(
        route.verify_and_decode_route_wire,
        _canonical(wire),
