"""Task-8 contract tests for the isolated C_UNION route."""
...
ROUTE_PATH = REPOSITORY_ROOT / "experiments/v3m0_b7_schema_lab/c_union.py"
ROUTE_ID = "C_UNION"
...
def _route_module():
    return importlib.import_module("experiments.v3m0_b7_schema_lab.c_union")
...
def _payload_fields(case_id: str) -> set[str]:
    bits = next(row[1] for row in CASE_ROWS if row[0] == case_id)
    return PAYLOAD_BASE_FIELDS | {
...
def test_c_union_exports_exact_bytes_only_api_and_passes_frozen_static_scan() -> None:
    route = _route_module()
    ...
    identifiers = {
        item.id.lower() for item in ast.walk(tree) if isinstance(item, ast.Name)
    } | {
        item.attr.lower() for item in ast.walk(tree) if isinstance(item, ast.Attribute)
    }
    assert not any(
        token in identifier
        for identifier in identifiers
        for token in ("callback", "leaf", "authority", "production", "dynamic")
    )
    ...
    assert set(common.compute_route_static_metrics_v1(ROUTE_ID, route_blob)) == {
...
def test_c_union_roundtrips_each_closed_terminal_variant(case_id: str) -> None:
    route = _route_module()
    source = _canonical(_transcript(case_id))
    ...
    assert wire["variant"] == case_id
    assert set(wire["payload"]) == _payload_fields(case_id)
    assert wire["transcript_sha"] == json.loads(source)["experimental_sha"]
    assert not any("event" in name.lower() for name in wire)
    assert not any("event" in name.lower() for name in wire["payload"])
...
def test_c_union_rejects_every_non_bytes_input(api_name: str, payload: object) -> None:
    route = _route_module()
...
def test_c_union_rejects_noncanonical_duplicate_concat_and_invalid_json(mutate) -> None:
    route = _route_module()
...
def test_c_union_rejects_unknown_or_missing_transcript_wire_and_payload_fields(
    operation: str,
) -> None:
    route = _route_module()
    ...
    if operation == "unknown":
        ...
        del wire["payload"]["actual_response_values"]
    _assert_rejected(
        route.verify_and_decode_route_wire,
        _resign_wire(wire),
        "PRESENCE_MISMATCH",
...
@pytest.mark.parametrize("case_id", [row[0] for row in CASE_ROWS])
def test_c_union_payload_field_set_is_exactly_the_nine_bit_presence_row(
    case_id: str,
) -> None:
    route = _route_module()
    wire = json.loads(
    ...
    assert [field in wire["payload"] for field in PRESENCE_FIELDS] == [
    ...
    for field in (
        ...
        if field in wire["payload"]:
            assert set(wire["payload"][field]) == {
...
def test_c_union_rejects_schema_and_case_domain_drift(
...
def test_c_union_rejects_every_self_hash_layer_drift(attack: str) -> None:
    route = _route_module()
    transcript = _transcript("success")
    if attack == "outer_hash":
        ...
        _seal(transcript, "experimental_sha")
    elif attack == "response_hash":
        transcript["actual_completed_response"]["response_sha"] = "0" * 64
        _seal(transcript, "experimental_sha")
    else:
...
def test_c_union_distinguishes_lineage_shas_from_declared_self_hashes() -> None:
    route = _route_module()
    transcript = _transcript("success")
...
@pytest.mark.parametrize("case_id", [row[0] for row in CASE_ROWS])
def test_c_union_rejects_presence_terminal_failure_and_schedule_mutations(
    case_id: str,
) -> None:
    route = _route_module()
    ...
    schedule = copy.deepcopy(transcript)
    schedule["callback_trace"] = list(reversed(schedule["callback_trace"]))
    _seal(schedule, "experimental_sha")
    if len(schedule["callback_trace"]) > 1:
        _assert_rejected(
            route.encode_normalized_transcript,
            _canonical(schedule),
...
@pytest.mark.parametrize(
    ...
    (
        "variant",
        "payload_variant",
        "wire_transcript_root",
        "shell_reference",
        "completed_values",
        "completed_audit",
        "completed_shell",
        "attempt_core_branch",
        "attempt_core_open_shape",
    ),
)
def test_c_union_decoder_rejects_discriminator_nested_evidence_and_cross_root_splices(
    attack: str,
) -> None:
    route = _route_module()
    wire = json.loads(
    ...
    if attack == "variant":
        wire["variant"] = "shell_failure"
    elif attack == "payload_variant":
        wire["payload"]["case_id"] = "shell_failure"
    elif attack == "wire_transcript_root":
        wire["transcript_sha"] = "0" * 64
    elif attack == "shell_reference":
        wire["payload"]["shell_outcome"]["reference_outcome"]["outcome_sha"] = "0" * 64
        _seal(wire["payload"]["shell_outcome"], "outcome_sha")
    elif attack == "completed_values":
        wire["payload"]["actual_completed_response"]["values"]["values_wire"] = [
            [1.0, 0.0]
        ]
        _seal(wire["payload"]["actual_completed_response"]["values"], "tensor_sha")
        _seal(wire["payload"]["actual_completed_response"], "response_sha")
    elif attack == "completed_audit":
        wire["payload"]["actual_completed_response"]["bridge_audit"] = copy.deepcopy(
            wire["payload"]["matched_ablated_bridge_audit"]
        )
        _seal(wire["payload"]["actual_completed_response"], "response_sha")
    elif attack == "completed_shell":
        wire["payload"]["actual_completed_response"]["shell_manifest_sha"] = "0" * 64
        _seal(wire["payload"]["actual_completed_response"], "response_sha")
    elif attack == "attempt_core_branch":
        wire["payload"]["actual_branch_attempt_core"]["branch"] = "matched_ablated"
    else:
        wire["payload"]["actual_branch_attempt_core"]["response_values"] = (
            copy.deepcopy(wire["payload"]["actual_response_values"])
        )
    _assert_rejected(
        route.verify_and_decode_route_wire,
        _resign_wire(wire),
        "PRESENCE_MISMATCH"
        if attack in ("variant", "payload_variant")
        else "EVIDENCE_MISMATCH",
    )


def test_c_union_rejects_open_event_list_and_cross_variant_payload_splice() -> None:
    route = _route_module()
    success = json.loads(
        route.encode_normalized_transcript(_canonical(_transcript("success")))
    )
    shell_failure = json.loads(
        route.encode_normalized_transcript(_canonical(_transcript("shell_failure")))
    )
    ...
    open_shape = {
        ...
        "variant": "success",
        "events": [],
        "transcript_sha": success["transcript_sha"],
        "wire_sha": "",
    }
    _seal(open_shape, "wire_sha")
    _assert_rejected(
        route.verify_and_decode_route_wire,
        _canonical(open_shape),
        "UNKNOWN_OR_MISSING_FIELD",
    )

    success["payload"] = copy.deepcopy(shell_failure["payload"])
    success["transcript_sha"] = shell_failure["transcript_sha"]
    _assert_rejected(
        route.verify_and_decode_route_wire,
        _resign_wire(success),
        "PRESENCE_MISMATCH",
    )


def test_c_union_source_has_no_open_event_or_unfrozen_public_surface() -> None:
    source = ROUTE_PATH.read_text(encoding="utf-8")
    lowered = source.lower()
    assert "event_list" not in lowered
    assert "append(" not in lowered
