...
def _validate_wire_value_v1(
    ...
    if nested_record is not None and (
        wire_type == nested_record
        or wire_type.startswith(f"{nested_record}[")
    ):
...
def validate_synthetic_parent_freeze_v3_body_v1(
    ...
    if (
        ...
        or candidate["candidate_schema_version"]
    ...
    for reference, (path, role, scope) in zip(references, source_specs):
        if (
            ...
            or reference["preparation_commit_sha"]
            != parent["preparation_commit_sha"]
    ...
    for receipt, review_role in zip(receipts, review_roles):
        if (
            ...
            or receipt["preparation_commit_sha"]
            != parent["preparation_commit_sha"]
...
def validate_synthetic_component_body_v1(
    ...
    for lineage_id in lineage_ids:
        ...
        parents[lineage_id] = lineage_matches[0]
...
def _validate_synthetic_graph_raw_v1(
    ...
    for component_id in component_order:
        ...
        if manifest[field_by_component[component_id]] != wrapper[
            "body_self_hash_value"
        ]:
    ...
    if manifest["synthetic_graph_component_root_sha"] != canonical_sha_v1(
        root_entries
    ):
...
def validate_response_run_spec_fixture_v1(
    ...
    if (
        ...
        raise ValueError("response run-spec Parent/permit/materialization lineage drifted")
    ...
    for name in ("source_basis", "readout_basis"):
        expected_name = (
            "scenario_source_basis" if name == "source_basis" else "scenario_readout_basis"
        )
        _canonical_equal_v1(
            spec[name],
    ...
    if spec["source_basis"]["role"] != "source" or spec["readout_basis"][
        "role"
    ] != "readout":
    ...
    if (
        ...
        or spec["preregistered_phase_bands"]
        ...
        or spec["source_trial_generation_id"]
        != contract["source_trial_generation_id"]
    ):
        raise ValueError("response run-spec endpoint/rank/trial contract drifted")
    if canonical_json_bytes_v1(spec["bridge_tolerance"]) != canonical_json_bytes_v1(
        contract["bridge_tolerance"]
    ) or spec["bridge_tolerance_source_id"] != contract[
    ...
    for component_id in (
        ...
        certificate_outcome = _component_by_id_v1(graph, component_id)[
            "complete_body"
        ]
        ...
        if (
            ...
            raise ValueError("response run-spec B6 certificate outcome is not successful")
    ...
    if (
        spec["actual_bridge_grid_authority_sha"]
        != actual_bridge["grid_authority_sha"]
...
def validate_source_readout_response_raw_v1(
    ...
    if (
        ...
        raise ValueError("source/readout response certificate lineage is not successful")
    ...
    if (
        ...
        raise ValueError("source/readout response factory/transition/certificate drifted")
    _canonical_equal_v1(
    ...
    if (
        ...
        or bridge["dynamics_certificate_sha"]
        != response["dynamics_certificate_sha"]
...
def _require_exact_dict_fields_v1(
    ...
    if tuple(raw_body) != fields:
        raise ValueError(f"{field} fields or field order drifted")
    return raw_body
